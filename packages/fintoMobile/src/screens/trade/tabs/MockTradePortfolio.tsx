...
import MockPortfolioMolecule from '@src/components/molecules/MockTradePortfolioMolecule/MockTradePortfolioMolecule';

export default function MockTradePortfolio() {
  const navigation = useNavigation();

  const portfolioRenderItem = ({item}: {item: any}) => {
    const onSellStcok = () => {
      navigation.navigate(RouteKeys.MOCKTRADESELLSTOCKSSCREEN)
    };

    return <MockPortfolioMolecule item={item} onSellStcok={onSellStcok} />;
  };
  return (
